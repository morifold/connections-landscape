 ...
 */
export function generateInteriorBoundaryPoints(
  ...
  for (let r = 0; r < H; r++) {
    ...
    points.push([left + width - dx, top + curvature + dy]);
  }
  for (let q = 0; q < W; q++) {
    let t = q / W;
    let dx = (width - 2 * curvature) * t;
    let dy = epsilon + curvature * 4 * (t - 0.5) ** 2;
    points.push([left + width - curvature - dx, top + height - dy]);
  }
 ...
 */
export function generateExteriorBoundaryPoints(
  { left, top, width, height },
  boundarySpacing: number
): Point[] {
  // https://www.redblobgames.com/x/2314-poisson-with-boundary/
